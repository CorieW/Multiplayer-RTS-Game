...
public class Unit : PlayerEntity
{
    ...
    const float RESOURCE_STORING_DISTANCE = 0.5f;
        ...
        foreach (Task task in _taskList.GetTasks())
        {
            switch (task.taskType)
            {
                ...
                case TaskType.Haul:
                    ...
                    return;
                case TaskType.Store:
                    PerformStore(task as StoreTask);
                    return;
    ...
    private void PerformMove(MoveTask moveTask)
    {
        ...
        if (_aiPath.destination != moveTask.pos) CmdPerformMove(moveTask.pos);
    ...
    {
        ResourceDeposit resDepo = resourceHarvestTask.resourceDeposit;
        if (resDepo == null) 
    ...
    private void PerformHaul(HaulTask haulTask)
    {
        ...
        ResourceDrop resDrop = haulTask.resourceDrop;
    ...
    [Client]
    private void PerformStore(StoreTask storeTask)
    {
        Stockpile stockpile = storeTask.stockpile;

        if (_hauling == null) 
        { // The unit is not hauling, so just finish the task.
            _taskList.FinishTask();
            return;
        }

        if (Vector2.Distance(transform.position, stockpile.transform.position) > RESOURCE_STORING_DISTANCE)
        {
           _taskList.AssignPriorityTask(
                new MoveTask(stockpile.transform.position - new Vector3(0, RESOURCE_STORING_DISTANCE))
            );
            return;
        }

        // Todo: Test
        stockpile.CmdStore(_hauling);
        // The hauling resource has been passed on and is therefore set to null.
        _hauling = null;
    }
